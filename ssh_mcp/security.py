...
class CommandValidator:
    ...
    def __init__(
        ...
        for risk_level, patterns in self.DANGEROUS_COMMAND_PATTERNS.items():
            self._risk_pattern_regex[risk_level] = [re.compile(pattern) for pattern in patterns]
                ...
                for category_cmds in section_data.values():
                    if isinstance(category_cmds, list):
                        extra_commands.update(
                            ...
                            if isinstance(cmd, str) and not cmd.startswith("_")
                        )
...
{"⚠️  重要提醒：" if risk_level in [RiskLevel.CRITICAL, RiskLevel.HIGH] else "💡 提示："}
   {
            ...
            else "此操作可能影响系统稳定性，请谨慎操作。"
            if risk_level == RiskLevel.HIGH
            else "此操作可能影响系统配置，建议先备份。"
            if risk_level == RiskLevel.MEDIUM
            else "此操作影响较小，但建议仔细检查参数。"
        }
...
"""
    ...
    def check_multi_layer_confirmation(
        self, command: str, confirm_dangerous: bool = False, current_layer: int = 1
    ) -> tuple[bool, str]:
        """
        检查多层安全确认
        ...
        return (
            True,
            f"✅ 已完成 {required_confirmations} 层安全确认，允许执行 {risk_level.value} 风险操作。",
        )
...
def create_validators_from_env():
    ...
    if config_file and os.path.exists(config_file):
        ...
        try:
            ...
            if extra_commands_str:
                extra_commands = set(
                    cmd.strip() for cmd in extra_commands_str.split(",") if cmd.strip()
                )
    ...
    if command_validator is None:
        ...
        command_validator = CommandValidator(
            security_level=security_level, extra_allowed_commands=extra_commands
        )

    # 读取基础目录
    base_dir = os.getenv("SSH_BASE_DIR", "/home")

    path_validator = PathValidator(security_level=security_level, base_dir=base_dir)
