...
class ParamikoClient(SSHClientInterface):
    ...
    def connect(self, timeout: int = 30) -> ConnectionResult:
        ...
        if self.is_connected:
            return ConnectionResult(success=True, message="Already connected")
        ...
        try:
            ...
            self._logger.info(
                f"Connected to {self.config.host}:{self.config.port} in {latency_ms:.2f}ms"
            )
            ...
            return ConnectionResult(success=False, message=f"Authentication failed: {str(e)}")
        except SSHException as e:
            self._logger.error(f"SSH error: {str(e)}")
            return ConnectionResult(success=False, message=f"SSH error: {str(e)}")
        except TimeoutError:
            self._logger.error(f"Connection timeout after {timeout}s")
            return ConnectionResult(success=False, message=f"Connection timeout after {timeout}s")
        except OSError as e:
            self._logger.error(f"Network error: {str(e)}")
            return ConnectionResult(success=False, message=f"Network error: {str(e)}")
    ...
    def execute_command(
        ...
        if not self.is_connected:
            raise CommandExecutionException("Not connected to SSH server", command=command)
        ...
        try:
            ...
            if background:
                ...
                return CommandResult(
                    stdout="Command started in background", stderr="", return_code=0
                )
            ...
            return CommandResult(stdout=stdout_data, stderr=stderr_data, return_code=return_code)

        except SSHException as e:
            self._logger.error(f"SSH error during command execution: {str(e)}")
            raise CommandExecutionException(
                f"SSH error: {str(e)}", command=command, original_error=e
            )
        except TimeoutError as e:
            self._logger.error(f"Command execution timeout after {timeout}s")
            raise CommandExecutionException(
                f"Command execution timeout after {timeout}s", command=command, original_error=e
            )
        except Exception as e:
            self._logger.error(f"Command execution failed: {str(e)}")
            raise CommandExecutionException(
                f"Failed to execute command: {str(e)}", command=command, original_error=e
            )
    ...
    def execute_command_stream(self, command: str) -> Iterator[str]:
        ...
        try:
            ...
            raise CommandExecutionException(
                f"Failed to execute streaming command: {str(e)}", command=command, original_error=e
            )
    ...
    def upload_file(self, local_path: str, remote_path: str) -> FileTransferResult:
        ...
        """
        if not self.is_connected:
            return FileTransferResult(success=False, message="Not connected to SSH server")
        ...
        try:
            ...
            return FileTransferResult(success=False, message=f"Upload failed: {str(e)}")
    ...
    def download_file(self, remote_path: str, local_path: str) -> FileTransferResult:
        ...
        try:
            ...
            return FileTransferResult(success=False, message=f"Download failed: {str(e)}")
    ...
    def get_transport_info(self) -> dict:
        ...
        return {
            ...
            "server_key_type": transport.get_remote_server_key().get_name()
            if transport and transport.get_remote_server_key()
            else None,
            ...
            "socket": {"timeout": self.config.timeout, "compress": self.config.compress},
        }
