...
SSH LICCO 目前仅支持 **AsyncSSH** 客户端，以提供更简洁、高性能的异步体验。

## 📦 支持的客户端类型

| 客户端 | 类型 | 特点 | 安装 | 状态 |
|--------|------|------|------|------|
| **AsyncSSH** | 异步 | 高并发性能，异步架构，现代化 API | `pip install asyncssh` | ✅ 默认且唯一支持 |

**说明：** 为了简化维护和优化性能，本项目从 0.1.0 版本开始仅支持 AsyncSSH 客户端。

## 🔧 配置方式

### 方式 1：在 MCP 配置中全局指定

在 MCP 配置文件中设置（可选，因为默认就是 AsyncSSH）：
